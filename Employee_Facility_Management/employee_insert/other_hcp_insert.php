...
<head>
    ...
    <title>Other HCP Registration</title>
    <style>
        body {
            padding-top: 50px;
        }
...
<body>
    <div class="container">
        <h1 class="text-center">Other HCP Registration</h1>
        <form method="post">
            <div class="row">
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($result3->num_rows == 0) {
        ...
        $Other_HCP_Employee_ID = $con->insert_id;

        // Insert nurse data
        $sql4 = "INSERT INTO OTHER_HCP (Other_HCP_Employee_ID, Job_Title)
             VALUES (?, ?)";
        $stmt4 = $con->prepare($sql4);
        $stmt4->bind_param("is", $Other_HCP_Employee_ID, $jobTitle);
        $stmt4->execute();

        if ($stmt2->affected_rows > 0 && $stmt4->affected_rows > 0) {
            echo '<script>alert("Other HCP data inserted successfully!")</script>';
        } else {
