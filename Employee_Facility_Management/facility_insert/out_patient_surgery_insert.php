...
// Fetch Facility_IDs that are in FACILITY table but not in OUT_PATIENT_SURGERY table
$sql = "SELECT f.Facility_ID FROM facility f LEFT JOIN out_patient_surgery o ON f.Facility_ID = o.Facility_ID WHERE o.Facility_ID IS NULL";
...
if ($result) {
    ...
    if (empty($facilityIDs)) {
        ...
        echo '<script>alert("No available Facility IDs.");</script>';
    }
} else {
...
<body>
    <h1 style="text-align: center;">Enter your details to get registered as Out Patient Surgery Facility</h1>
    <form style="margin: 40px;" method="post">
        ...
        <div class="form-group col-md-2">
            <label for="p_code">Enter P_Code</label>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($result2->execute()) {
        echo '<script>alert("Out Patient Surgery Facility inserted successfully!")</script>';
    } else {
